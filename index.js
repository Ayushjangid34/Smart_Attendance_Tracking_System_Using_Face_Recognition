...
// Serve static files from the FrontendPages folder
app.use(express.static(path.join(__dirname, 'FrontendPages')));


const storage = multer.diskStorage({
  destination: function (req, file, cb) {
    cb(null, path.join(__dirname, 'StudentImages'))
  },
...
const upload = multer({ storage: storage,
  fileFilter: function (req, file, cb) {
    ...
    if (extname && mimetype) {
        ...
        cb(new Error('Only images with jpeg, jpg, png extensions are allowed'));
    }
} 
})
...
// Defining a route
app.get('/', (req, res) => {
    res.sendFile(path.join(__dirname, 'FrontendPages', 'Home.html'));
});
...
app.post('/admin-login', (req, res) => {
    ...
    pool.query('SELECT * FROM Admins WHERE Email = ? AND Password = ?', [email, password], (error, results) => {
        ...
        res.sendFile(path.join(__dirname, 'FrontendPages', 'admin.html'));
  ...
  app.post('/faculty-login', (req, res) => {
    ...
    pool.query('SELECT * FROM Faculties WHERE Email = ? AND Password = ?', [email, password], (error, results) => {
        ...
        if (results.length === 1) {
        // User found, login successful
        res.sendFile(path.join(__dirname, 'FrontendPages', 'faculty.html'));
        console.log('Admin Login successful!');
  ...
  app.post('/student-login', (req, res) => {
    ...
    pool.query('SELECT * FROM Students WHERE Email = ? AND Password = ?', [email, password], (error, results) => {
        ...
        if (results.length === 1) {
        // User found, login successful
        res.sendFile(path.join(__dirname, 'FrontendPages', 'student.html'));
...
app.post('/student-photo', upload.single('imageFile'), (req, res) => {
  ...
  let childProcess = exec(
    'python ' + path.join(__dirname, 'FaceRecognition', 'encodeStudent.py'),
    (error, stdout, stderr) => {
      if (error) {
...
app.use('/images', express.static(path.join(__dirname, 'StudentImages')));
